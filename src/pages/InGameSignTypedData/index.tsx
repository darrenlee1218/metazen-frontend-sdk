...
import { signTypedData } from '@services/APIServices/wallet';
import {
  bridgeDataEmitter,
...
export const InGameSignTypedDataPage: FC = () => {
  ...
  const [chainId, address, typedData] = params as [chainId: string, address: string, typedData: EIP712TypedData];
  ...
  const onSignClick = (): void => {
    setIsLoading(true);
    (async () => {
      const errors = validations(chainId, address, typedData);
      if (errors.length > 0) {
        setErrorMessage(errors);
        setIsLoading(false);
        return;
      }
      const result = await signTypedData(chainId, address, typedData);
    ...
    <main>
      <DefaultErrorBoundary>
        <Container maxWidth="sm" sx={{ px: 4, pt: '16px', pb: '16px', height: '100%' }}>
          ...
          <InGameSigningPageContent type={InGameSigningType.TYPED_DATA} typedData={typedData} />
